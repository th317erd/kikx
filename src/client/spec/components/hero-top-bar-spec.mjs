...
function registerComponent() {
  ...
  class HeroTopBar extends JsdomHTMLElement {
    ...
    _onSettingsClick() {
      navigateCalls.push({ path: '/hero/settings' });
    }

    _onLogoutClick() {
      profileLogoutCalls.push(true);
      navigateCalls.push({ path: '/hero/login' });
    }
  }

  dom.window.customElements.define('hero-top-bar', HeroTopBar);
}
...
describe('hero-top-bar', () => {
  let element;

  beforeEach(() => {
    resetMocks();
    setupDOM();
    element = dom.window.document.createElement('hero-top-bar');
  ...
  it('registers as a custom element', () => {
    let registered = dom.window.customElements.get('hero-top-bar');
    assert.ok(registered, 'hero-top-bar should be registered as a custom element');
  });
  ...
  it('calls profile.logout and navigates to login when Logout button is clicked', () => {
    ...
    assert.equal(navigateCalls[0].path, '/hero/login');
  });
  ...
  it('navigates to /hero/ when back button is clicked', () => {
    let backButton = element.shadowRoot.querySelector('.back-button');
    backButton.click();

    assert.equal(navigateCalls.length, 1, 'navigate should have been called');
    assert.equal(navigateCalls[0].path, '/hero/');
  });
  ...
  it('navigates to /hero/settings when settings button is clicked', () => {
    let settingsButton = element.shadowRoot.querySelector('.settings-button');
    settingsButton.click();

    assert.equal(navigateCalls.length, 1, 'navigate should have been called');
    assert.equal(navigateCalls[0].path, '/hero/settings');
  });
  ...
  it('real module exports a class constructor', async () => {
    ...
    try {
      let mod = await import('../../components/hero-top-bar/hero-top-bar.mjs');
