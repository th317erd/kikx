...
function registerComponent() {
  ...
  class HeroMessageInput extends JsdomHTMLElement {
  ...
  dom.window.customElements.define('hero-message-input', HeroMessageInput);
}
...
describe('hero-message-input', () => {
  let element;

  beforeEach(() => {
    setupDOM();
    element = dom.window.document.createElement('hero-message-input');
  ...
  it('registers as a custom element', () => {
    let registered = dom.window.customElements.get('hero-message-input');
    assert.ok(registered, 'hero-message-input should be registered as a custom element');
  });
  ...
  it('real module exports a class constructor', async () => {
    ...
    try {
      let mod = await import('../../components/hero-message-input/hero-message-input.mjs');
