...
function makeParticipants() {
  return [
    ...
    { id: 'p3', name: 'HeroBot',   initials: 'HB', color: '#4caf50', role: 'assistant',   isBot: true },
  ];
}
...
function registerComponent() {
  ...
  class HeroParticipantList extends JsdomHTMLElement {
    constructor() {
      super();
  ...
  dom.window.customElements.define('hero-participant-list', HeroParticipantList);
}
...
describe('hero-participant-list', () => {
  let element;

  beforeEach(() => {
    setupDOM();
    element = dom.window.document.createElement('hero-participant-list');
  ...
  it('registers as a custom element', () => {
    let registered = dom.window.customElements.get('hero-participant-list');
    assert.ok(registered, 'hero-participant-list should be registered as a custom element');
  });
  ...
  it('displays participant names correctly', () => {
    ...
    assert.equal(names[2].textContent, 'HeroBot', 'third name should be HeroBot');
  });
  ...
  it('displays participant roles correctly', () => {
    ...
    assert.equal(roles[2].textContent, 'BOT', 'HeroBot role should be BOT');
  });
  ...
  it('applies coordinator-badge class to coordinator role', () => {
    ...
    // HeroBot -- should NOT have the badge
  ...
  it('real module exports a class constructor', async () => {
    ...
    try {
      let mod = await import('../../components/hero-participant-list/hero-participant-list.mjs');
