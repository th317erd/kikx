...
function registerComponent() {
  ...
  class HeroLoginPage extends JsdomHTMLElement {
    constructor() {
      super();
    ...
    async _onSubmit(event) {
      ...
      try {
        ...
        navigateCalls.push({ path: '/hero/', replace: true });
      } catch (error) {
  ...
  dom.window.customElements.define('hero-login-page', HeroLoginPage);
}
...
describe('hero-login-page', () => {
  let element;

  beforeEach(() => {
    resetMocks();
    setupDOM();
    element = dom.window.document.createElement('hero-login-page');
  ...
  it('registers as a custom element', () => {
    let registered = dom.window.customElements.get('hero-login-page');
    assert.ok(registered, 'hero-login-page should be registered as a custom element');
  });
  ...
  it('real module exports a class constructor', async () => {
    ...
    try {
      let mod = await import('../../components/hero-login-page/hero-login-page.mjs');
  ...
  it('renders the application title', () => {
    ...
    assert.equal(title.textContent, 'Hero');
  });
  ...
  it('calls sendMagicLink, sets token, updates store, and navigates on successful login', async () => {
    ...
    // Verify navigation to /hero/ with replace
