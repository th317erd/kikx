...
describe('hero-application', () => {
  ...
  before(async () => {
    ...
    let componentModule = await import('../../components/hero-application/hero-application.mjs');
    ...
    HeroApplication = componentModule.default;
  });
  ...
  beforeEach(() => {
    ...
    dom.window.history.replaceState(null, '', '/hero/');

    // Clear the body for a fresh DOM.
    dom.window.document.body.innerHTML = '';

    element = dom.window.document.createElement('hero-application');
  });
  ...
  it('registers as a custom element', () => {
    let Constructor = dom.window.customElements.get('hero-application');
    assert.ok(Constructor);
    assert.equal(Constructor, HeroApplication);
  });
  ...
  it('renders hero-login-page when route changes to login', () => {
    dom.window.document.body.appendChild(element);

    navigate('/hero/login');

    let page = element.querySelector('hero-login-page');
    assert.ok(page, 'expected hero-login-page to be rendered');
  });
  ...
  it('renders sessions page placeholder when route changes to sessions', () => {
    ...
    let page = element.querySelector('hero-session-page');
    assert.ok(page, 'expected hero-session-page to be rendered');
  });
  ...
  it('renders sessions page placeholder with params when route changes to session', () => {
    ...
    navigate('/hero/sessions/xyz-789');
  ...
  it('renders settings page placeholder when route changes to settings', () => {
    ...
    let page = element.querySelector('hero-settings-page');
    assert.ok(page, 'expected hero-settings-page to be rendered');
  });
  ...
  it('redirects to login when not authenticated and navigating to a protected route', () => {
    ...
    let page = element.querySelector('hero-login-page');
    assert.ok(page, 'expected hero-login-page after redirect');
  });
  ...
  it('allows navigation to protected routes when authenticated', () => {
    ...
    let page = element.querySelector('hero-settings-page');
    assert.ok(page, 'expected hero-settings-page when authenticated');
  });
  ...
  it('clears previous page when switching routes', () => {
    ...
    navigate('/hero/settings');
    assert.ok(element.querySelector('hero-settings-page'));

    navigate('/hero/');
    assert.ok(element.querySelector('hero-session-page'));
    assert.equal(element.querySelector('hero-settings-page'), null, 'previous page should be removed');
