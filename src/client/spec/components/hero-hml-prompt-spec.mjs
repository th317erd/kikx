...
function registerComponent() {
  ...
  class HeroHmlPrompt extends JsdomHTMLElement {
  ...
  dom.window.customElements.define('hero-hml-prompt', HeroHmlPrompt);
}
...
describe('hero-hml-prompt', () => {
  let element;

  beforeEach(() => {
    setupDOM();
    element = dom.window.document.createElement('hero-hml-prompt');
  ...
  it('registers as a custom element', () => {
    let registered = dom.window.customElements.get('hero-hml-prompt');
    assert.ok(registered, 'hero-hml-prompt should be registered as a custom element');
  });
