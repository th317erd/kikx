...
function setupDOM() {
  dom = new JSDOM('<!DOCTYPE html><html><body></body></html>', {
    url: 'http://localhost/kikx/settings',
...
function registerComponent() {
  ...
  dom.window.customElements.define('kikx-settings-page', KikxSettingsPage);
}
...
describe('kikx-settings-page', () => {
  let element;

  beforeEach(() => {
    setupDOM();
    element = dom.window.document.createElement('kikx-settings-page');
  ...
  it('registers as a custom element', () => {
    let registered = dom.window.customElements.get('kikx-settings-page');
    assert.ok(registered, 'kikx-settings-page should be registered as a custom element');
  });
  ...
  // -----------------------------------------------------------------------
  // 9. Back button dispatches navigate event with path /kikx/
  // -----------------------------------------------------------------------

  it('back button dispatches navigate event with path /kikx/', () => {
    let events = [];
    ...
    assert.equal(events[0].detail.path, '/kikx/', 'navigate event should have path /kikx/');
  ...
  it('real module exports a class constructor', async () => {
    ...
    try {
      let mod = await import('../../components/kikx-settings-page/kikx-settings-page.mjs');
