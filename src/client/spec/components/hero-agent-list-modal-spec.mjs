...
// wrapper since jsdom does not have hero-modal registered. The card list,
...
function registerComponent() {
  ...
  class HeroAgentListModal extends JsdomHTMLElement {
  ...
  dom.window.customElements.define('hero-agent-list-modal', HeroAgentListModal);
}
...
describe('hero-agent-list-modal', () => {
  let element;

  beforeEach(() => {
    setupDOM();
    element = dom.window.document.createElement('hero-agent-list-modal');
  ...
  it('registers as a custom element', () => {
    let registered = dom.window.customElements.get('hero-agent-list-modal');
    assert.ok(registered, 'hero-agent-list-modal should be registered as a custom element');
  });
  ...
  it('real module exports a class constructor', async () => {
    ...
    try {
      let mod = await import('../../components/hero-agent-list-modal/hero-agent-list-modal.mjs');
