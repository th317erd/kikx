...
function registerComponent() {
  ...
  class HeroCommandResult extends JsdomHTMLElement {
  ...
  dom.window.customElements.define('hero-command-result', HeroCommandResult);
}
...
describe('hero-command-result', () => {
  let element;

  beforeEach(() => {
    setupDOM();
    element = dom.window.document.createElement('hero-command-result');
  ...
  it('registers as a custom element', () => {
    let registered = dom.window.customElements.get('hero-command-result');
    assert.ok(registered, 'hero-command-result should be registered as a custom element');
  });
  ...
  it('real module exports a class constructor', async () => {
    ...
    try {
      let mod = await import('../../components/hero-command-result/hero-command-result.mjs');
