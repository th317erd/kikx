...
function registerComponent() {
  ...
  class HeroWebsearchResult extends JsdomHTMLElement {
  ...
  dom.window.customElements.define('hero-websearch-result', HeroWebsearchResult);
}
...
describe('hero-websearch-result', () => {
  let element;

  beforeEach(() => {
    setupDOM();
    element = dom.window.document.createElement('hero-websearch-result');
  ...
  it('registers as a custom element', () => {
    let registered = dom.window.customElements.get('hero-websearch-result');
    assert.ok(registered, 'hero-websearch-result should be registered as a custom element');
  });
  ...
  it('real module exports a class constructor', async () => {
    ...
    try {
      let mod = await import('../../components/hero-websearch-result/hero-websearch-result.mjs');
