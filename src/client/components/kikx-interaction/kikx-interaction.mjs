...
const TEMPLATE_HTML = `
  <style>
    ...
    /* ----------------------------------------------------------------- */
    ${glowInitCSS('kikx-interaction .bubble')}
    ${glowHoverCSS('kikx-interaction .bubble:hover:not(.focused)')}
    ${glowCSS('kikx-interaction .bubble.focused')}

    /* ----------------------------------------------------------------- */
    /* Glass-surface reflection (toggled via console: kikxReflections())  */
    /* ----------------------------------------------------------------- */
    kikx-interaction[reflect] .bubble {
      box-shadow:
    ...
    kikx-interaction[alignment="user"][reflect] .bubble {
      box-shadow:
    ...
    kikx-interaction[bubble-type="permission"][reflect] .bubble {
      box-shadow:
    ...
    kikx-interaction[bubble-type="error"][reflect] .bubble {
      box-shadow:
    ...
    kikx-interaction[alignment="user"] .bubble {
      background:
    ...
    /* ----------------------------------------------------------------- */
    kikx-interaction[bubble-type="permission"] .bubble {
      background:
        linear-gradient(135deg,
    ...
    kikx-interaction[bubble-type="permission"] .bubble-header {
      margin: -12px -14px 0;
      padding: 10px 14px;
    ...
    /* ----------------------------------------------------------------- */
    kikx-interaction[bubble-type="error"] .bubble {
      background:
        linear-gradient(135deg,
    ...
    kikx-interaction[bubble-type="error"] .bubble-header {
      margin: -12px -14px 0;
      padding: 10px 14px;
      border-radius: var(--border-radius-large, 12px) var(--border-radius-large, 12px) 0 0;
      background:
    ...
    kikx-interaction .bubble-header {
      display: flex;
      align-items: center;
      gap: var(--spacing-sm, 8px);
    }

    kikx-interaction .avatar {
      width: 28px;
      height: 28px;
      border-radius: 50%;
      display: flex;
    ...
    kikx-interaction .header-name {
      font-size: 1rem;
      font-weight: 600;
      color: var(--text-primary, #e8e8f0);
    }

    kikx-interaction .content {
      padding: 2px 0;
    }

    kikx-interaction .footer {
      display: flex;
      align-items: center;
    ...
    kikx-interaction .footer:empty {
      display: none;
    }

    kikx-interaction .reply-context {
      display: none;
      align-items: center;
      gap: var(--spacing-xs, 4px);
      padding: 4px 8px;
    ...
    kikx-interaction[parent-preview] .reply-context {
      display: flex;
    }

    kikx-interaction .reply-context-text {
      white-space: nowrap;
      overflow: hidden;
      text-overflow: ellipsis;
    }

    kikx-interaction .footer-left {
      display: flex;
      gap: var(--spacing-sm, 8px);
      align-items: center;
    }

    kikx-interaction .reply-button {
      border: none;
    ...
    kikx-interaction[data-frame-id] .reply-button {
      display: inline-flex;
      align-items: center;
      gap: 4px;
    }
    ...
    kikx-interaction .reply-button:hover {
      color: var(--accent-primary, #00e5ff);
      background: var(--glass-hover, rgba(255, 255, 255, 0.08));
    }

    kikx-interaction .reply-count-badge {
      display: none;
      font-size: 0.8rem;
      color: var(--accent-primary, #00e5ff);
      cursor: pointer;
    ...
    kikx-interaction .reply-count-badge:hover {
      background: var(--glass-hover, rgba(255, 255, 255, 0.08));
    }

    kikx-interaction[reply-count] .reply-count-badge {
      display: inline-flex;
      align-items: center;
      gap: 4px;
    }

    kikx-interaction .footer-right {
      display: flex;
      gap: var(--spacing-xs, 4px);
      align-items: center;
    }

    kikx-interaction .action-button {
      border: none;
      border-radius: var(--border-radius-small, 4px);
      padding: 8px 20px;
      font-size: 1rem;
    ...
    kikx-interaction .ignore-button {
      background: var(--glass-background, rgba(255, 255, 255, 0.05));
      color: var(--text-primary, #e8e8f0);
    }

    kikx-interaction .ignore-button:hover {
      box-shadow: 0 0 8px rgba(255, 255, 255, 0.12);
    }

    kikx-interaction .submit-button {
      background: var(--accent-primary, #00e5ff);
      color: #fff;
    }

    kikx-interaction .submit-button:hover {
...
class KikxInteraction extends HTMLElement {
  ...
  connectedCallback() {
    if (!this._initialized) {
      ...
      this._headerName       = this.querySelector('.header-name');
      this._footerMeta       = this.querySelector('.footer-meta');
      this._footerRight      = this.querySelector('.footer-right');
      this._replyButton      = this.querySelector('.reply-button');
...
if (typeof window !== 'undefined') {
  ...
  window.kikxReflections = function kikxReflections(force) {
    ...
    let count = 0;
    for (let el of document.querySelectorAll('kikx-interaction')) {
      if (enabled)
