...
## Current Work: Solr Indexing & Search — COMPLETE (2026-03-20)

- **Plan:** `bot-docs/plan/kikx/solr-indexing-and-search.yaml` (7 planning rounds)
- **Solr 9.10.1** running via Docker (port 8983, core `kikx`)
- **Schema:** `solr/kikx/conf/schema.xml` — 13 unified fields (content, note as text_html; archived replaces deleted)
- **SolrService:** `src/core/lib/solr-service.mjs` — fetch-based, 58 tests
- **Frame indexing:** `Frame.getContentForIndexing()` → `solr-document-mapper` → `SolrIndexingPlugin` (FrameRouter, type:* selector, best-effort)
- **ValueStore indexing:** `ValueStore.onAfterSave()` lifecycle hook — indexes ALL ValueStore records automatically
- **Document mapper:** `src/core/lib/solr-document-mapper.mjs` — mapFrameToSolrDocuments(), mapValueStoreToSolrDocument()
- **Search API:** `src/server/controllers/search-controller.mjs` — POST /api/v2/search + /api/v2/sessions/:sessionID/search
  - DB content enrichment with preview (1024 char max, contentRange: [0, 1024] or null)
  - Session permission filtering via sessionIDs param
  - Graceful Solr/DB error handling (503 for Solr down)
- **Agent tools:**
  - `search:query` — new internal plugin (`src/core/internal-plugins/search/index.mjs`), rows capped at 50, chunk-fetch hints
  - `tool_log:search` — re-backed with Solr, falls back to SQLite if unavailable
- **Client:** Sidebar search input wired to API (300ms debounce, session filtering by search results)
- **New tests:** 233 tests across 7 test files
- **Total test count:** ~4000+ tests, 0 failures
