...
export class KikxApp extends KikxComponent {
  static tagName = 'kikx-app';
  ...
  // Note: kikx-app uses Light DOM - children are directly in the element, not slotted.
  // The template in kikx-app.html is for reference/documentation only.
  ...
  mounted() {
    ...
    this.addEventListener('kikx:logout', () => this.logout());
    this.addEventListener('kikx:navigate', (event) => this.navigate(event.detail.path));
    this.addEventListener('kikx:authenticated', () => this.navigate('/'));
    this.addEventListener('kikx:show-modal', (event) => this._handleShowModal(event.detail));
    this.addEventListener('show-modal', (event) => this._handleShowModal(event.detail));
    this.addEventListener('kikx:clear-messages', () => this._handleClearMessages());
    this.addEventListener('kikx:toggle-hidden', (event) => this._handleToggleHidden(event.detail));
  ...
  _showView(viewName, options) {
    ...
    // Pass tab to kikx-settings when settings view is active
    if (viewName === 'settings') {
      let settingsComponent = this.querySelector('kikx-settings');
   ...
   */
  _connectWebSocket() {
    // WebSocket connection will be handled by kikx-websocket component
