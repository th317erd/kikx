...
/**
 * Kikx Input - Message Input Component
 *
 * Features:
...
export class KikxInput extends KikxComponent {
  static tagName = 'kikx-input';
  ...
  async processQueue() {
    while (this.#messageQueue.length > 0 && !this.#isLoading) {
      ...
      this.dispatchEvent(new CustomEvent('kikx:queue-process', {
      ...
      await new Promise((resolve) => {
        this.addEventListener('kikx:queue-complete', resolve, { once: true });
      });
    }
  }
   ...
   */
  queueComplete() {
    this.dispatchEvent(new CustomEvent('kikx:queue-complete'));
  }
   ...
   */
  handleClear() {
    this.dispatchEvent(new CustomEvent('kikx:clear', {
      bubbles: true,
      composed: true,
    }));
  }
  ...
  _queueMessage(content) {
    ...
    this.dispatchEvent(new CustomEvent('kikx:message-queued', {
  ...
  async _sendMessage(content) {
    ...
    try {
      this.dispatchEvent(new CustomEvent('kikx:send-message', {
        detail: {
          content,
  ...
  _draftKey() {
    ...
    return sessionId ? `kikx-draft-${sessionId}` : null;
  }
